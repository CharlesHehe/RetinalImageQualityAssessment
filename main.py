...
# image_datasets = {x: CustomImageDataset(os.path.join(data_dir, x), data_transforms[x]) for x in [train, val]}
...
#  fine_tuning we will be updating all parameters. However, if we are
...
# Train and evaluate
model_ft, hist = train_model(model_ft, dataloaders_dict, criterion, optimizer_ft, scheduler,
...
plt.title(model_name + " Validation Accuracy vs. Number of Training Epochs")
...
plt.savefig(f"./val_acc/{model_name}_{str(batch_size)}_{str(num_epochs)}.png")
plt.legend()
plt.show()

torch.save(model_ft.state_dict(), f"./trained_model/{model_name}_{str(batch_size)}_{str(num_epochs)}.pth")
